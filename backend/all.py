...
class School():
  def __init__(self, school_boundary, blocked_out_areas, num_students):
    ...
    self.coords = self.sample_lat_long(num_students)[:2000]
...
class LocIdentification():
  def __init__(self, coords, cluster_labels, all_data):
    ...
    self.locations = []
    self.absolute_peaks = []
    self.predicted_loc = self.calculate(coords[:2000], cluster_labels, all_data[:2000])
...
def predict_location():
  ...
  count += 1
  print(shooter_points[count][0], shooter_points[count][1])
  return shooter_points[count][0], shooter_points[count][1]
